...
export const checkAiContent = async (
    ...
    const analysisPrompt = `Perform a deep linguistic analysis on the following text to determine if it was generated by an AI. Your analysis should go beyond surface-level patterns.

Consider the following factors in your evaluation:
1. **Perplexity and Predictability:** Assess whether the word choices are overly predictable or simplistic, common in AI-generated text.
2. **Sentence Structure Variety:** Analyze the diversity in sentence length, structure (simple, compound, complex), and beginnings. AI text often lacks this variety.
3. **Linguistic Nuance:** Look for subtle signs of human writing, such as idioms used correctly, nuanced emotional tone, and colloquialisms.
4. **Consistency and Flow:** Check for unnatural transitions or a robotic consistency in tone.

Based on this comprehensive analysis, break the text down into segments. For each segment, determine if it is AI-generated and provide a confidence score from 0.0 to 1.0.
...
export const summarizeText = async (
    ...
    let promptCore = "Provide a concise paragraph summarizing the key points of the following text segment.";
    
    switch (length) {
        case 'short':
            promptCore = "Summarize the following text segment into a few key bullet points. Ensure the summary is brief and captures only the most critical information.";
            break;
        case 'long':
            promptCore = "Provide a detailed summary of the following text segment. It should be comprehensive, covering the main arguments and conclusions, while being substantially shorter than the original.";
            break;
    }
    ...
    for (let i = 0; i < totalChunks; i++) {
        const chunk = chunks[i];
        const progress = ((i + 1) / totalChunks) * 100;
        const baseMessage = `Summarizing chunk ${i + 1} of ${totalChunks}...`;
        onProgress(progress, baseMessage);

        try {
            const request: GenerateContentParameters = {
                model: 'gemini-2.5-flash',
                contents: `${promptCore} Respond only with the summarized text. Here is the text to summarize: "${chunk}"`,
            };
            ...
            const response = await generateContentWithRetry(request, onRetry);
            summarizedChunks.push(response.text);
